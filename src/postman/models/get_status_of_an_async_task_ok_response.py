...
@JsonMap({})
class GetStatusOfAnAsyncTaskOkResponseMeta(BaseModel):
...
class GetStatusOfAnAsyncTaskOkResponseStatus(Enum):
    """An enumeration representing different categories.
    ...
    def list():
        ...
        return list(
            map(
                lambda x: x.value,
                GetStatusOfAnAsyncTaskOkResponseStatus._member_map_.values(),
            )
        )
...
class Details1(BaseModel):
    ...
    def __init__(self, resources: List[Resources] = None):
...
class GetStatusOfAnAsyncTaskOkResponse(BaseModel):
    ...
    :type meta: GetStatusOfAnAsyncTaskOkResponseMeta, optional
    :param status: The task's current status., defaults to None
    :type status: GetStatusOfAnAsyncTaskOkResponseStatus, optional
    ...
    def __init__(
        self,
        id_: str = None,
        meta: GetStatusOfAnAsyncTaskOkResponseMeta = None,
        status: GetStatusOfAnAsyncTaskOkResponseStatus = None,
        ...
        if id_ is not None:
            self.id_ = id_
        if meta is not None:
            self.meta = self._define_object(meta, GetStatusOfAnAsyncTaskOkResponseMeta)
        if status is not None:
            self.status = self._enum_matching(
                status, GetStatusOfAnAsyncTaskOkResponseStatus.list(), "status"
            )
