...
@JsonMap({})
class UpdateCollectionRequestOkResponse(BaseModel):
    """UpdateCollectionRequestOkResponse

    :param data: Information about the updated request. For a complete list of properties, refer to the `definitions.request` property in the [collection.json schema file](https://schema.postman.com/collection/json/v1.0.0/draft-07/collection.json)., defaults to None
